...
public class DemoAction2 extends SceneAction {
  //--- Attributes ---
  ...
  //--- Constructor ---
  public DemoAction2() throws java.rmi.RemoteException {}
  public DemoAction2(Scene scene, Action superSceneAction, int direct, int lane) throws java.rmi.RemoteException {
    this.name = "DemoAction2_"+direct+"."+lane;
    this.scene = scene;
    ...
    int endDirect = (direct + 1 + lane * 2) % 4;
    ...
    moves[1] = DemoScene.move6[direct][lane];
  ...
  protected void sceneActionScript() {
    try {
      if ( (externalTickets == null ) || (externalTickets.length != 2) ) {
        System.out.println("DemoAction2 was called incompletely");
        return;
      }
      ...
      if (DEBUG) System.out.println("*D* DemoAction2: move1 raised anticipating sign");
      // claim ticCross2
      ticCross2.insist();
      ...
      if (DEBUG) System.out.println("*D* DemoAction2: move1 raised finished sign");
      ...
      if (DEBUG) System.out.println("*D* DemoAction2: move2 raised anticipating sign");
      move2.getManoeuvre().getPrecaution(0).remove();
      // wait for finishedSign of move2
      watch(move2.getSign(1));
      if (DEBUG) System.out.println("*D* DemoAction2: move2 raised finished sign");
      // raise a sign
