...
public class DemoAction5 extends SceneAction {
  //--- Attributes ---
  ...
  //--- Constructor ---
  public DemoAction5() throws java.rmi.RemoteException {}
  public DemoAction5(Scene scene, Action superSceneAction, int direct, int lane) throws java.rmi.RemoteException {
    this.name = "DemoAction5_"+direct+"."+lane;
    this.scene = scene;
    ...
    //tickets[3] = new PrimeTicket(this, DemoScene.semEndPark[(direct + 2) % 4][(lane + 1) % 2]);//end-park

    this.moves = new Move[2];
    moves[0] = DemoScene.move3[direct][lane];
    moves[1] = DemoScene.move6[(direct + 3 - lane*2) % 4][(lane + 1) % 2];
  ...
  protected void sceneActionScript() {
    try {
      if ( (externalTickets == null ) || (externalTickets.length != 2) ) {
        System.out.println("DemoAction5 was called incompletely");
        return;
      }
      ...
      if (DEBUG) System.out.println("*D* DemoAction5: move1 raised anticipating sign");
      // claim ticCross2
      ticCross2.insist();
      ...
      if (DEBUG) System.out.println("*D* DemoAction5: move1 raised finished sign");
      ...
      if (DEBUG) System.out.println("*D* DemoAction5: move2 raised anticipating sign");
      move2.getManoeuvre().getPrecaution(0).remove();
      // wait for finishedSign of move2
      watch(move2.getSign(1));
      if (DEBUG) System.out.println("*D* DemoAction5: move2 raised finished sign");
      // raise a sign
      ...
      System.out.println("Exception in DemoAction5: The scene-action will be aborted.");
      e.printStackTrace();
    }
