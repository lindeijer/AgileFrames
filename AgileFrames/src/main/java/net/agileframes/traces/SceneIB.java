...
public class SceneIB extends ServerIB implements Scene/*, Cloneable*/ {
  ...
  /**
   * Indicates if this Scene is changed.<p>
  ...
  /**
   * An array of the sub-scenes belonging to this Scene.<p>
   * The subScenes in this array will be set as soon as they are available.
   * When they are not available anymore, they will be set to null.
   * All this is done by the RemoteSceneListener's method notify(re).
  ...
  /**
   * An array of the class-names of the sub-scenes belonging to this Scene.<p>
   * Must be set in the constructor of the specific Scene. This array is needed
   * to lookup the Scenes by the RemoteSceneListener.<br>
   * If this Scene does not have any sub-scenes then this array should be null.
   * @see RemoteSceneListener
   */
  protected Class[] subSceneClasses = null;
  /**
   * An array of the positions of the sub-scenes belonging to this Scene.<p>
   * Must be set in the constructor of the specific Scene. This array is needed
   * to lookup the Scenes by the RemoteSceneListener.<br>
   * If this Scene does not have any sub-scenes then this array should be null.
   * @see RemoteSceneListener
  ...
  /**
   * Uploads this Scene to the Jini Lookup Services.<p>
   * Creates the RemoteSceneListener and check for sub-scenes already available.<br>
  ...
  public void uploadScene() {
    if (subScenePositions != null) {
      ...
      for (int i = 0; i < subScenePositions.length; i++) {
        subScenePositions[i] = subScenePositions[i].transformT1T2(transform, subScenePositions[i]);//right order? or should it be (t, transform) ??
      }
  ...
  /**
   * Initializes the Scene.<p>
   * Should be implemented in the User-defined Scenes.
   * Should create the semaphores, scene-actions, logistic-positions and moves.
   */
  ...
  /**
   * Makes a clone of a scene-action in this Scene and returns it.<p>
   * Searches a SceneAction that matches the given description.<br>
   * <b><i>Make sure that after calling this method, you should call setActor on the SceneAction!</b></i>
   * Registers the SceneAction and the Actor so that when the actor dies, the Scene
   * can take care of destroying the SceneAction.<br>
   * The name of the SceneAction is set in its Constructor. It is a convention that
   * with parametrized SceneActions dots (.) and underscores(_) are being used. For example:<br>
   * <blockquote>FromAToBAction_1.0.3</blockquote>
   * @see     net.agileframes.core.traces.SceneAction#setActor(Actor)
   * @param   name  the description of the SceneAction to download
   * @param   actor the actor that requests the download
  ...
  public synchronized SceneAction getSceneAction(String name, Actor actor) throws java.rmi.RemoteException {
    ...
    for (int i = 0; i < sceneActions.length; i++) {
      if (sceneActions[i].getName().equals(name)) { action = sceneActions[i]; break; }
    }
    if (action == null) {
  ...
  public synchronized FuTrajectory[] getSceneTrajects() throws RemoteException {
    if (moves == null) { return new FuTrajectory[] {}; }
  ...
  /**
   * Returns all the scene-actions vbelonging to this Scene.<p>
  ...
  private synchronized void registerSA(Actor actor, SceneAction clonedAction) {
    // if the clonedAction has a non-null super-action, we have to clone that one as well
    SceneAction thisAction = clonedAction;
    Action superAction = clonedAction.superSceneAction;
    ServiceID thisID = null;
    try { thisID = actor.getServiceID(); }
    ...
    int index = -1;
    for (int i = 0; i < actorIDs.length; i++) { if (actorIDs[i].equals(thisID)) { index = i; break; } }
    if (index == -1) {// never seen this actor before
      ...
      for (int i = 0; i < actorIDs.length; i++) { newActorIDs[i] = actorIDs[i]; newActions[i] = actions[i]; newPrevActions[i] = prevActions[i]; }
      ...
      for (int i = 0; i < actorIDs.length; i++) { actorIDs[i] = newActorIDs[i]; actions[i] = newActions[i]; prevActions[i] = newPrevActions[i]; }
  ...
  /**
   * Destroys the SceneAction(s) of an Actor.<p>
   * Necessary when an Actor dies and without cleaning its scene-actions.
   * Called in ActorIB or in superScene.destroySceneAction.
   * Calls subScenes.destroySceneAction and kills the last 2 sceneactions in this scene that
   * are downloaded by the actor.
   * @see   net.agileframes.core.traces.SceneAction#dispose()
   * @param actor the actor of which the SceneActions should be destroyed
   */
  public synchronized void destroySceneAction(Actor actor) throws RemoteException {
    if (subScenes != null) {
      for (int i = 0; i < subScenes.length; i++) {
        if (subScenes[i] != null) { subScenes[i].destroySceneAction(actor); }
      }
    }
    SceneAction action = null;
    ServiceID actorID = null;
    try { actorID = actor.getServiceID(); } catch (RemoteException e) { e.printStackTrace(); }
    for (int i = 0; i < actorIDs.length; i++) {
      if (actorIDs[i].equals(actorID)) {
        // it is necessay to destroy the previous action because we are not certain that its exit-ticket is freed
        if (prevActions[i] != null) { prevActions[i].dispose(); }
        else { System.out.println("Scene.destroySceneAction: Actor not known"); }
        if (actions[i] != null) { actions[i].dispose(); }
        else { System.out.println("Scene.destroySceneAction: Actor not known"); }
        break;
      }
    }
  ...
  public void setSubScene(Scene subScene, int index, RemoteSceneListener listener) throws RemoteException {
    ...
    if (subScene != null) { subScene.setSuperScene(this); }
    setSAs();
  }

  /**
   * Sets the (dynamic) SceneActions of this Scene.<p>
   * To be implemented in specific user-defined (super-)Scenes.
   * When Scene-Actions are defined where sub-Scenes are needed, they do not become
   * valid until the sub-Scenes are available. This method is called in setSubScene().
      ...
      if (logisticPositions != null) {
        for (int i = 0; i < logisticPositions.length; i++) {
          if (logisticPositions[i] != null) {
            ...
            if (d < minDist) { minDist = d; lp = logisticPositions[i]; }
          }
        }
      }
        ...
        for (int i=0; i < subScenes.length; i++) {
          if (subScenes[i] != null) {
            ...
            if (subLp != null) {
              ...
              if (d < minDist) { minDist = d; lp = subLp;}
            }
          }
        }
      }
...
//inherited from scene
  public synchronized SceneAction join(Actor actor, LogisticPosition lp) throws java.rmi.RemoteException {
    ...
    try { sa =(SceneAction)sa.clone(actor); }
    catch (Exception e) { e.printStackTrace(); }
    registerSA(actor, sa);
  ...
  public Scene getTopScene() throws java.rmi.RemoteException {
    if (superScene != null) { return superScene.getTopScene(); }
    else { return this; }
  }
  //inherited from scene
      ...
      if (sceneActions != null) {
        for (int i = 0; i < sceneActions.length; i++) {
          if (sceneActions[i] != null) {
            ...
            if (sceneActions[i].getName().equals(sceneActionName)) { return this; }
          } else { System.out.println("sa "+i+" is null"); }
        }
      }
      if (subScenes != null) {
        for (int i = 0; i < subScenes.length; i++) {
          if (subScenes[i] != null) {
            if (subScenes[i].find(sceneActionName) != null) { return subScenes[i]; }
          }
        }
      }
    } catch (Exception e) { e.printStackTrace(); }
