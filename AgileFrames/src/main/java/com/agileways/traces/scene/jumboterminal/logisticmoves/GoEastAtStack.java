...
/**
 * The move that goes one position further eastwards at the top-lane so that it ends
 * above the stack and lane specified in the constructor. This move is proceeded by another
 * GoEastAtStack or a Turn and followed by another GoEastAtStack or an EnterStackWest.
 *
 * @author Wierenga
 * @version 0.0.1
 */

public class GoEastAtStack extends MoveImplBase{
   /**
   *
   ...
   */
  public GoEastAtStack(int stackNr, int stackLane, int lanePosition) throws java.rmi.RemoteException {
    // Create transform to determine starting location
    float x;
    if ((stackNr==0) && (stackLane==0)) {
      x = CrossoverScene.cT.FREE_TERMINAL_LEFT+ CrossoverScene.cT.TURN_RADIUS + CrossoverScene.cT.WIDTH_DRIVING_LANE;
    } else {
      if (stackLane==0) {
        x = CrossoverScene.cT.getStacklaneX(stackNr-1,CrossoverScene.cT.lanesPerStack-1)- CrossoverScene.cT.TURN_RADIUS;
      } else {
  ...
  public void createRules(){
    ...
    this.rules = new Rule[] {new Move.EvolutionRule(this, trajectory, trajectory.domain - CrossoverScene.cT.AGV_LENGTH, 0)};
  }
  ...
  public synchronized void event(int eventID,int seqNum) {
    switch (seqNum) {
      ...
      default:
        try{
          entryTickets[seqNum-1].free();
        }
        catch(Exception e) {System.out.println("Exception while entryTickets["+(seqNum-1)+"].free() in GoEastAtStack:"+e.getMessage());}
        break;
    }
  }
