...
/**
 * The move that starts at the toplane and enters the stack from the east. This move is proceeded
 * by GoWestAtStack and followed by LeaveStackForParking, LeaveStackForCenterEast or LeaveStackForCenterWest.
 *
 * @author Wierenga
 * @version 0.0.1
 */
public class EnterStackEast extends MoveImplBase{
   ...
   */
  public EnterStackEast(int stackNr, int stackLane, int lanePosition) throws java.rmi.RemoteException {
    ...
    composedTrajectory[0] = new CircularBendLeft(CrossoverScene.cT.TURN_RADIUS, CrossoverScene.cT.AGV_TURNING_POINT);
  ...
  public synchronized void event(int eventID,int seqNum) {
    switch (seqNum) {
      ...
      case 1:// stack free
        try{
          entryTickets[1].free();//stac
          entryTickets[0].free();//totalstack
        }
        catch(Exception e) {System.out.println("Exception while entryTickets[0].free() in EnterStackEast:"+e.getMessage());}
        break;
      case 2:// entrance and crossinglane
        try{this.entryTickets[2].free();
        if (this.entryTickets[3]!=null) {entryTickets[3].free();}
        }
        catch(Exception e) {System.out.println("Exception while entryTickets[1].free() in EnterStackEast:"+e.getMessage());}
        break;
