...
public class SceneIB extends ServerIB implements Scene/* , Cloneable */ {
	...
	public void uploadScene() {
		...
		try {
			...
			Entry[] attributeSets = new Entry[] { new Name(this.getName()), getAvatarFactory(),new TransformEntry(transform) };
	...
	private synchronized void registerSA(Actor actor, Action clonedAction) {
		...
		Action superAction = thisAction.superSceneAction;
		...
		if (index == -1) {// never seen this actor before
			...
			newActions[newActorIDs.length - 1] = thisAction;
